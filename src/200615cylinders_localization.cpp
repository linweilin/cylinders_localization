...
double AngleBetweenTwoLines (Line &l1, Line &l2)
{
    ...
    double cos_numerator = abs( v1.dot(v2) );
    double cos_denominator_L = v1.norm();
    double cos_denominator_R = v2.norm();
...
//         cout << "original orientation is: " << source_lines.at(n).GetOrientation().transpose() << endl;
        if (dist_pte_pte > dist_pte_pts)
        {
//             source_lines.at(n).ReverseOrientation();
            reverse_lines.at(n).ReverseOrientation();
//             cout << "reverse orientation is: " << source_lines.at(n).GetOrientation().transpose() << endl;            
        }
    }
}
